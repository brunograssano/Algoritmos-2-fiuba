#include "abb.h"
const int ERROR_A=-1,EXITO_A=0,NO_EXISTE_A=0;
const int ES_MAYOR_A=1,SON_IGUALES_A=0,ES_MENOR_A=-1;
...
nodo_abb_t* insertar_elemento(nodo_abb_t* nodo,abb_comparador comparador,void* elemento,int* estado){
	...
	if(resultado==ES_MAYOR_A || resultado==SON_IGUALES_A){
...
nodo_abb_t* borrar_elemento(abb_comparador comparador,abb_liberar_elemento destructor,nodo_abb_t* nodo,void* elemento,int* estado){
	...
	int resultado=comparador(elemento,nodo->elemento);
	if(resultado==SON_IGUALES_A){
		return reacomodar_arbol(nodo,destructor,estado);
	}
	else if(resultado==ES_MAYOR_A){
 ...
 */
void inorden_con_cada_elemento(nodo_abb_t* nodo, bool (*funcion)(void*, void*, void*), void* extra,void* extra2,bool* estado){
	if(nodo==NULL){
		return;
	}
	if(*estado){
	...
	inorden_con_cada_elemento(nodo->izquierda,funcion,extra,extra2,estado);
	if(*estado){
		return;
	}
	(*estado)=(*funcion)(nodo->elemento,extra,extra2);
	inorden_con_cada_elemento(nodo->derecha,funcion,extra,extra2,estado);
}

/* Recorre el arbol de forma preorden hasta que lo recorra entero o que la funcion devuelva true.
 */
void preorden_con_cada_elemento(nodo_abb_t* nodo, bool (*funcion)(void*, void*, void*), void* extra,void* extra2,bool* estado){
	if(nodo==NULL){
		return;
	}
	if(*estado){
	...
	(*estado)=(*funcion)(nodo->elemento,extra,extra2);
	preorden_con_cada_elemento(nodo->izquierda,funcion,extra,extra2,estado);
	if(*estado){
		return;
	}
	preorden_con_cada_elemento(nodo->derecha,funcion,extra,extra2,estado);
}

/* Recorre el arbol de forma postorden hasta que lo recorra entero o que la funcion devuelva true.
 */
void postorden_con_cada_elemento(nodo_abb_t* nodo, bool (*funcion)(void*, void*, void*), void* extra, void* extra2,bool* estado){
	if(nodo==NULL){
		return;
	}
	if(*estado){
	...
	postorden_con_cada_elemento(nodo->izquierda,funcion,extra,extra2,estado);
	if(*estado){
		return;
	}
	postorden_con_cada_elemento(nodo->derecha,funcion,extra,extra2,estado);
	if(*estado){
		return;
	}
	(*estado)=(*funcion)(nodo->elemento,extra,extra2);
...
*/
void abb_con_cada_elemento(abb_t* arbol, int recorrido, bool (*funcion)(void*, void*, void*), void* extra,void* extra2){
	if(arbol==NULL || funcion==NULL){
		return;
	}
	bool estado=false;
	if(recorrido==ABB_RECORRER_INORDEN){
		inorden_con_cada_elemento(arbol->nodo_raiz,funcion,extra,extra2,&estado);
	}
	else if(recorrido==ABB_RECORRER_PREORDEN){
		preorden_con_cada_elemento(arbol->nodo_raiz,funcion,extra,extra2,&estado);
	}
	else if(recorrido==ABB_RECORRER_POSTORDEN){
		postorden_con_cada_elemento(arbol->nodo_raiz,funcion,extra,extra2,&estado);
	}
}
